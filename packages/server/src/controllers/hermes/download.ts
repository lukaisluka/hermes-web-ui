...
function isForbiddenDownloadPath(ctx: Context, filePath: string): boolean {
...
export async function downloadFile(ctx: Context) {
  const filePath = (ctx.query as any).path as string | undefined
  const fileName = (ctx.query as any).name as string | undefined
  ...
  try {
    ...
    if (isRegularUser((ctx.state as any)?.user) &&
